...
void SDRAM_GPIOConfig(void)
{
	...
	//SDCLK : PG8
	//SDCKE1 : PB5
	//D0~D15 : PD14~15, PD0~1, PE7~15, PD8~10
	//NBL0~1 : PE0~1
	//const uint8_t PortB_PinList[] = { 5 };
	//const uint8_t PortD_PinList[] = { 0, 1, 8, 9, 10, 14, 15 };
	//const uint8_t PortE_PinList[] = { 0, 1, 7, 8, 9, 10, 11, 12, 13, 14, 15 };
	//const uint8_t PortF_PinList[] = { 0, 1, 2, 3, 4, 5, 11, 12, 13, 14, 15 };
	//const uint8_t PortG_PinList[] = { 0, 1, 4, 5, 8, 15 };
	//const uint8_t PortH_PinList[] = { 5, 6 };

	////const uint32_t pinConfig = 0x3C2;    // Speed 100Mhz, AF12 FSMC, Alternate Mode
	//const uint32_t pinConfig = 0x2C2;    // Speed 50Mhz, AF12 FSMC, Alternate Mode
	//const uint32_t pinMode = pinConfig & 0xF;
	//const GPIO_ALT_MODE alternateMode = (GPIO_ALT_MODE)pinConfig;
	//const GPIO_RESISTOR resistorConfig = RESISTOR_DISABLED;

	///* Initialize SDRAM */
	//for (uint32_t i = 0; i < ARRAY_LENGTH(PortB_PinList); i++)
	//{
	//	CPU_GPIO_ReservePin(PORT_PIN(GPIO_PORTB, PortB_PinList[i]), TRUE);
	//	CPU_GPIO_DisablePin(PORT_PIN(GPIO_PORTB, PortB_PinList[i]), resistorConfig, 0, alternateMode);
	//	STM32F4_GPIO_Pin_Config(PORT_PIN(GPIO_PORTB, PortB_PinList[i]), pinMode, resistorConfig, pinConfig); // Workaround, since CPU_GPIO_DisablePin() does not correctly initialize pin speeds
	//}
	//for (uint32_t i = 0; i < ARRAY_LENGTH(PortD_PinList); i++)
	//{
	...
	//}
	//for (uint32_t i = 0; i < ARRAY_LENGTH(PortE_PinList); i++)
	//{
	...
	//}
	//for (uint32_t i = 0; i < ARRAY_LENGTH(PortF_PinList); i++)
	//{
	...
	//}
	//for (uint32_t i = 0; i < ARRAY_LENGTH(PortG_PinList); i++)
	//{
	...
	//}
	//for (uint32_t i = 0; i < ARRAY_LENGTH(PortH_PinList); i++)
	//{
	//	CPU_GPIO_ReservePin(PORT_PIN(GPIO_PORTH, PortH_PinList[i]), TRUE);
	//	CPU_GPIO_DisablePin(PORT_PIN(GPIO_PORTH, PortH_PinList[i]), resistorConfig, 0, alternateMode);
	//	STM32F4_GPIO_Pin_Config(PORT_PIN(GPIO_PORTH, PortH_PinList[i]), pinMode, resistorConfig, pinConfig); // Workaround, since CPU_GPIO_DisablePin() does not correctly initialize pin speeds
	//}
	...
	/* Enable GPIOs clock */
	//RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB | RCC_AHB1Periph_GPIOH | RCC_AHB1Periph_GPIOD |
...
void SDRAM_InitSequence(void)
{
	...
	FMC_SetRefreshCount(1292);//84MHz
	//FMC_SetRefreshCount(1386);//90MHz
	/* Wait until the SDRAM controller is ready */
	while (FMC_GetFlagStatus(FMC_Bank2_SDRAM, FMC_FLAG_Busy) != RESET)
	{
	}
